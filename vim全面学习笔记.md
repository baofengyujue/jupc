...
- x/X应该是删除光标处字符或者光标处前一位字符
- s/S为删除光标处字符或者光标所处行，并进入插入模式
- r/3r进入替换模式，键入一个字符替换当前光标处字符然后重回命令模式///进入替换模式，键入一个字符将光标处向后3个字符都替换为键入字符，然后重回命令模式
- 替换模式简单说明一下，就是输入字符将当前光标处的字符替换为键入字符，而不是插入模式当中的插入新字符
- J/2J使下一/2行合并到当前行
